/* ImageMask — catalogue de 10 formes SVG `<clipPath>` réutilisables.
 *
 * Source : composant 21st.dev "image-mask" intégré verbatim côté géométrie
 * (paths SVG inchangés). Refactor uniquement la structure :
 *   - `<defs>` extraits dans `ImageMaskDefs` à mount UNE FOIS au niveau layout
 *     (sinon collisions d'IDs si plusieurs instances dans la page).
 *   - `MaskedImage` = wrapper paramétré (props `src` + `pattern`) → piochable
 *     sur n'importe quelle image GND, n'importe quelle page.
 *   - `IMAGE_MASK_PATTERNS` constant + type `ImageMaskPattern` pour typage strict
 *     des id pattern dispos (autocomplete dans le code utilisateur).
 *
 * Les 10 patterns dispos (référence visuelle) :
 *   - clip-pattern   : rect avec coin top-right grignoté + arche bas
 *   - clip-pattern1  : rect avec bord bas ondulé (4 bumps)
 *   - clip-pattern2  : tombstone (dôme arrondi en haut)
 *   - clip-pattern3  : rect avec encoche bottom-left + arche top
 *   - clip-pattern4  : ovale / ellipse plein
 *   - clip-pattern5  : rect avec accolades latérales
 *   - clip-pattern6  : rect avec dôme inversé au centre du bas
 *   - clip-pattern7  : rect avec coin bottom-left grignoté
 *   - clip-pattern8  : rect avec liseré festonné en haut
 *   - clip-inverted  : forme bandeau wide (16/9-ish) avec encoches asymétriques
 */
import * as React from 'react';

export const IMAGE_MASK_PATTERNS = [
  'clip-pattern',
  'clip-pattern1',
  'clip-pattern2',
  'clip-pattern3',
...
export type ImageMaskPattern = typeof IMAGE_MASK_PATTERNS[number];

/**
 * Mount UNE FOIS dans le layout (RefonteApp). Ne rend rien visible —
 * SVG placé en absolute hors-écran avec dimensions 0 (les <clipPath> sont
 * référençables globalement via leur id).
 */
export function ImageMaskDefs() {
  return (
    <svg
      aria-hidden="true"
...
type MaskedImageProps = {
  src: string;
  alt?: string;
  pattern: ImageMaskPattern;
  /** Classe sur le <figure> wrapper (taille, position). */
  className?: string;
  /** Classe sur le <img> intérieur (object-fit, hover effects). Override le défaut. */
  imgClassName?: string;
  loading?: 'lazy' | 'eager';
};

/**
 * Image individuelle clipée par un pattern SVG.
 * Requiert <ImageMaskDefs /> mounté UNE FOIS dans le layout.
 *
 * Exemple :
 *   <MaskedImage
...
export function MaskedImage({
  ...
  return (
    <figure className={className} style={{ clipPath: `url(#${pattern})` }}>
      <img
        src={src}
        alt={alt}
        loading={loading}
        className={
          imgClassName ??
          'transition-all duration-500 ease-out aspect-[4/5] min-h-full align-bottom object-cover hover:scale-110 w-full h-full'
        }
      />
    </figure>
  );
